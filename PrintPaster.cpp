...
void PrintPaster::PrinterEngine()
{

    //the frame construction using positions of objects
	score+=difficulty;
	Screen << '\n' << "\tJUMPS    : " ;
	for(int z = 0; z < 3-objFend.nofdigits(jumps); z++)
    {
        Screen << '0';
    }
	...
	Screen << '\n' << "\tHI JUMPS : ";
	for(int z = 0; z < 3-objFend.nofdigits(HiJumps); z++)
    {
        Screen << '0';
    }
...
void PrintPaster::screenDisplay()
{
    Screen.seekp(0);
    while(!Screen.eof())
    {
